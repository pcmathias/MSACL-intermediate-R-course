---
title: 'Lesson 9: Predictions using linear regression'
output:
...
```{r}
my_slr_fit <- lm(igfrc ~ sdma, ckd)

tidy(my_slr_fit) #equation: iGFRc = -24.7*SDMA + 73.5
glance(my_slr_fit) #R2: 0.48
```
...
ckd_test <- ckd_test %>%
              mutate(igfr_pred = round(predict(mod_full, ckd_test),0))
...
```{r}
# iGFRc = 0.42*Trp - 2.99*Kynurenine - 0.49*BUN - 5.4*CYC_DB - 14.7*ADMA - 0.09*Phe + 75.16
```
...
```{r}
glance(mod2)$r.squared #0.851
rmse(ckd_train$igfrc, mod2$fitted.values) #7.48
mape(ckd_train$igfrc, mod2$fitted.values) #0.15 or 15%
