...
2. Load the here package from within your console and use the `set_here()` command to set your "coursework" directory to be the primary point of reference for your project. (This creates an empty file named ".here", which is the first thing the here package looks for when trying to set its point of reference.)
3. Create the directory structure we discussed in the last lesson, **with the exception of the data directory***
4. Go to your operating system and copy the "class_data" directory you unzipped prior to the course (per the pre-course instructions) into your "coursework" directory 
5. Before we jump into creating our R Markdown file, we need to create a file that will allow us to run checkpoint together with knitr - click the Add File button and select "R Script"
6. Add the following code to your R script (minus the code chunk delimiters)
```{r}
library("formatR")
library("htmltools")
library("caTools")
...
library("stringi")
```
7. Save as "knitr-packages.R" in the src folder of your project (explanation of why you did this below)
8. Now add a new R Markdown file, title it "Data Science 201 Exercises", and leave as the default .html ouptut selection
9. Go ahead and save the .Rmd by clicking the Save button (disk button) or selecing Save under the File menu: save the file to your src folder within your project and name according to the principles we disucssed in lesson 1
10. In the setup code chunk for the document, load the checkpoint package
11. Add the checkpoint command to load packages from today: `checkpoint("2018-01-21")`
12. Load the here package below the checkpoint command
13. Modify the header of the first block of text to "Lesson 3 Exercises"
