...
Because these commands can duplicate rows, there is the potential of breaking things pretty significantly if the key isn't unique. Here's are two examples, one where you do, and do not want that duplication:
```{r good_and_bad_duplication}
goodDuplication <- inner_join(x=bySample[,c(1:3,7)],y=byBatch[,c(1:6)],by=c("batchName","compoundName"))
badDuplication <- inner_join(x=bySample[,c(1:3,7)],y=byBatch[,c(1:6)],by=c("compoundName"))
```
...
byPeak <- left_join(byPeak,byBatch,by=c("batchName","analyte"="compoundName"))
```
