...
1. OK, let's get in the habit of working within our project structure. Copy the class data you downloaded into the data folder of your project.
1. Use the base `read.csv()` function to read the "2017-01-06.csv" file into a data frame.
...
```{r base_load}
base_load <- read.csv(here("data", "2017-01-06.csv"))
str(base_load)
summary(base_load)
```
...
```{r base_load_nofactors}
base_load_nofactors <- read.csv(here("data", "2017-01-06.csv"), stringsAsFactors = FALSE)
...
1. What does the data look like? (Recall the `summary()` function to view column types and characteristics about the data.)
```{r readr_load}
readr_load <- read_csv(here("data", "2017-01-06.csv"))
str(readr_load)
summary(readr_load)
```
...
**Time to read with base:** `r system.time(base_load <- read.csv(here("data", "2017-01-06.csv")))`

**Time to read with readr:** `r system.time(readr_load <- read_csv(here("data", "2017-01-06.csv")))`

Finally, let's follow some best practices and explicitly define columns with the `col_types` argument. We want to explicitly define batchName, compoundName, and sampleType as factors. Note that the `col_factor()` expects a definition of the factor levels but you can get around this by supplying a `NULL`. Then run a summary to review the data
```{r col_types}
readr_load_factors <- read_csv(
  here("data", "2017-01-06.csv"),
  col_types = cols(
    batchName = col_factor(NULL),
...
```{r readxl}
readxl_load <- read_excel(here("data", "2017-01-06.csv"))
summary(readxl_load)
readxl_load_subset <- read_excel(here("data", "2017-01-06.csv"), range = cell_cols("A:F"))
head(readxl_load_subset)
```

## Importing dirty data

To close out the discussion on reading files, there is one more useful package to introduce that helps with a variety of data cleaning functions. Since this is R, the package is cleverly and appropriately named [janitor](https://github.com/sfirke/janitor).